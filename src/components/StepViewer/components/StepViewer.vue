<template>
  <div class="step-viewer" ref="viewerRef">
    ...
    <Toolbar
:file-name="store.currentFileName" :is-loading="store.isLoading" :has-model="store.hasModel"
    ...
    <div class="viewer-content">
      <URDFLeftPanel
v-if="store.hasModel" ref="urdfLeftPanelRef" @export-urdf="handleExportURDF"
        @rotate-to-z-up="handleRotateToZUp" @reset-orientation="handleResetOrientation" />

      <SidePanel
:visible="modelTreeVisible" @tree-select="handleTreeSelect" @solid-hover="handleSolidHover"
        @toggle-solid-visibility="handleToggleSolidVisibility" @close="modelTreeVisible = false" />

      <MeasurementPanel
:visible="measurePanelVisible" @remove="handleRemoveMeasurement"
        @clear-all="handleClearMeasurements" @close="measurePanelVisible = false" />

      <div class="canvas-container" ref="canvasContainerRef">
        <StatsPanel
:visible="showStats" :triangles="modelTriangles" :vertices="modelVertices"
          :draw-calls="frameDrawCalls" ref="statsPanelRef" />

        <LoadingOverlay
:visible="store.isLoading" :progress="store.uploadProgress.progress"
...
function handleViewShortcut(e: KeyboardEvent): void {
  ...
  switch (e.key) {
    case 'x': sceneManager.setViewPreset(ViewPreset.RIGHT); break
    case 'X': sceneManager.setViewPreset(ViewPreset.LEFT); break
    case 'y': sceneManager.setViewPreset(ViewPreset.TOP); break
    case 'Y': sceneManager.setViewPreset(ViewPreset.BOTTOM); break
    case 'z': sceneManager.setViewPreset(ViewPreset.FRONT); break
    case 'Z': sceneManager.setViewPreset(ViewPreset.BACK); break
