<template>
  <div class="view-controls">
    ...
    <div class="control-row axis-row">
      ...
      <el-slider
v-model="urdfStore.axisHelperScale" :min="0.1" :max="5" :step="0.1" :show-tooltip="true"
    ...
    <div class="zup-section">
      <div class="control-row">
        ...
        <el-tag v-if="stepStore.isModelRotated" size="small" type="success" effect="light">已旋转</el-tag>
      </div>
      ...
      <div class="control-row zup-actions">
        <el-tooltip
          ...
          placement="top">
          <el-button
            type="warning" plain size="small" :disabled="!stepStore.hasModel"
            @click="emit('rotateToZUp', currentUpAxis)">
            🧭 旋转到 Z-up
          </el-button>
        </el-tooltip>
        <el-button
          text size="small" :disabled="!stepStore.isModelRotated"
      ...
      <el-tooltip
        ...
        placement="top">
        <el-button
          text type="primary" size="small" style="width: 100%; margin-top: 2px"
  ...
  <el-dialog v-model="inertiaDialogVisible" title="整机惯量计算" width="780px" :close-on-click-modal="false" append-to-body>
    <div class="inertia-dialog-body">
      ...
      <div class="param-row">
        ...
        <el-input-number
v-model="totalMass" :min="0.001" :max="100000" :precision="3" :step="1"
            ...
            <template #default="{ row }">
              <div class="solid-detail">
                ...
                <div v-for="s in row.solids" :key="s.solidId" class="solid-mass-row">
                  ...
                  <el-input-number
v-model="s.mass" :min="0.0001" :max="100000" :precision="4" :step="0.05"
                    controls-position="right" style="width: 132px" @change="recalcRow(row as ResultRow)" />
          ...
          <el-table-column prop="name" label="连杆" min-width="90" show-overflow-tooltip align="center" />
          ...
          <el-table-column label="质量 (kg)" width="152" align="center">
            <template #default="{ row }">
              <el-input-number
v-model="row.mass" :min="0.0001" :max="100000" :precision="4" :step="0.1"
                controls-position="right" style="width: 136px" @change="onLinkMassChange(row as ResultRow)" />
            </template>
          </el-table-column>
        ...
        <p class="edit-hint">
          修改连杆质量会按当前各 Solid 质量比例同步缩放；单独修改 Solid 质量则连杆质量取各 Solid 之和。
        </p>
      </div>
    </div>
...
import { UP_AXIS_OPTIONS, alignAllJointFramesToWorldZUp, type UpAxis } from '../../core/ZUpTransform'
...
const totalMass = computed({
  ...
  set: (v: number) => { urdfStore.totalMass = v }
})
...
function redistributeByVolume(): void {
  const totalVolume = computedResults.value.reduce(
    (s, r) => s + r.solids.reduce((v, e) => v + e.volume, 0), 0
  )
