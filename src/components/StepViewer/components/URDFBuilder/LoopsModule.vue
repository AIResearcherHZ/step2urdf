<template>
  <div class="loops-module">
    <div class="module-header">
      ...
      <el-button size="small" type="primary" text :disabled="urdfStore.robot.links.length < 2" @click="handleAdd">
        + 添加
      </el-button>
    </div>

    <div v-if="urdfStore.robot.loops.length === 0" class="empty-hint">
      暂无闭链约束。URDF 为树结构无法表达闭链，此处定义的约束会写入 MuJoCo robot.xml 的 &lt;equality&gt; 段。
    </div>
    ...
    <div v-for="loop in urdfStore.robot.loops" :key="loop.id" class="loop-item">
      <div class="loop-row">
        ...
        <el-button size="small" type="danger" text :icon="Delete" @click="urdfStore.removeLoop(loop.id)" />
      </div>
      ...
      <div v-if="loop.type === 'connect'" class="loop-row">
        ...
        <el-input-number
v-model="loop.anchor[0]" size="small" :step="1" :precision="3" controls-position="right"
          style="width: 76px" />
        <el-input-number
v-model="loop.anchor[1]" size="small" :step="1" :precision="3" controls-position="right"
          style="width: 76px" />
        <el-input-number
v-model="loop.anchor[2]" size="small" :step="1" :precision="3" controls-position="right"
          style="width: 76px" />
      </div>
      ...
      <div class="loop-row">
        ...
        <el-input-number
v-model="loop.solref[0]" size="small" :step="0.005" :precision="4" controls-position="right"
          style="width: 92px" />
        <el-input-number
v-model="loop.solref[1]" size="small" :step="0.1" :precision="3" controls-position="right"
          style="width: 92px" />
      </div>
    </div>
  </div>
