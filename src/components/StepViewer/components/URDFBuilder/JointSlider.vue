<template>
  <div v-if="joint.type === 'ball'" class="ball-group">
    ...
    <div v-for="(label, i) in BALL_LABELS" :key="label" class="joint-slider">
      ...
      <el-slider
:model-value="ballValue[i]" :min="-ballRange" :max="ballRange" :step="ballStep" :show-tooltip="false"
  ...
  <div v-else class="joint-slider">
    <span class="slider-name" :title="joint.name">{{ joint.name }}</span>
    <el-slider
:model-value="joint.currentValue" :min="joint.limits.lower" :max="joint.limits.upper" :step="sliderStep"
