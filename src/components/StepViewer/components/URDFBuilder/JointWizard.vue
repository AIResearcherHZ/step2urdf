...
v-for="l in availableChildLinks" :key="l.id" :label="l.name" :value="l.id"
        ...
        <div class="candidate-block">
          <div class="candidate-head">
            ...
            <el-tag v-if="candidates.length" size="small" type="info">{{ candidates.length }} 个</el-tag>
            <el-button size="small" text :disabled="!canDetect" @click="detectCandidates">重新识别</el-button>
          </div>
          <div v-if="!canDetect" class="candidate-empty">请先选择 Parent 与 Child</div>
          <div v-else-if="candidates.length === 0" class="candidate-empty">未识别到同轴特征，请在 3D 中手动拾取</div>
          <ul v-else class="candidate-list">
            <li
v-for="(c, i) in candidates" :key="c.id" class="candidate-item"
              :class="{ active: activeCandidateId === c.id }" @mouseenter="handleCandidateHover(c)"
              @mouseleave="handleCandidateLeave" @click="applyCandidate(c)">
        ...
        <div class="pick-hint">
          ...
          <div class="pick-tools">
            ...
            <span class="pick-cycle">
              ...
              <el-button size="small" text :disabled="candidateInfo.total < 2" @click="emit('cycleCandidate', 1)">
                切换 (Tab)
              </el-button>
            </span>
          </div>
          <div v-if="candidateInfo.description" class="pick-current">当前: {{ candidateInfo.description }}</div>
        ...
        <div v-if="hasSnap" class="offset-block">
          <div class="field-row">
            ...
            <el-slider
v-model="axisOffset" :min="offsetRange[0]" :max="offsetRange[1]" :step="offsetStep"
              size="small" style="flex:1;margin-right:8px" @input="refreshGizmo" @change="handleOffsetChange" />
            <el-input-number
v-model="axisOffset" size="small" :step="offsetStep" :precision="3"
              controls-position="right" style="width:110px" @change="handleOffsetChange" />
          </div>
          <div v-if="snapPointOptions.length" class="field-row">
            <span class="field-label">吸附:</span>
            <el-select
v-model="selectedSnapPoint" size="small" style="flex:1" placeholder="吸附到同轴特征"
              clearable @change="handleSnapPointChange">
              <el-option v-for="(sp, i) in snapPointOptions" :key="i" :label="sp.label" :value="sp.t" />
            </el-select>
          </div>
        </div>
