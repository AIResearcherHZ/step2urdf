...
export function serializeMJCF(robot: URDFRobot, options?: MJCFSerializeOptions): string {
  ...
  const meshLinks = options?.meshLinkNames
  ...
  function serializeBody(
    ...
    lines.push(`${pad}<body name="${escapeXml(link.name)}" pos="${fmtVec3(pose.xyz)}" quat="${fmtVec4(pose.quat)}">`)
    ...
    for (const loop of loops) {
      ...
      if (loop.type === 'connect') {
        ...
        const anchorScaled: [number, number, number] = [
          anchor[0] * scale, anchor[1] * scale, anchor[2] * scale
        ]
        lines.push(
