...
export function buildAxisFrame(normal: [number, number, number]): AxisFrame {
  ...
  const ref: [number, number, number] = absX < absY && absX < absZ
    ? [1, 0, 0]
    : absY < absZ
      ? [0, 1, 0]
      : [0, 0, 1]
...
export function frameToArray(frame: AxisFrame): [  number, number, number,
  number, number, number,
  number, number, number
] {
  return [
...
export function flipRPY(
  ...
  axis: FrameAxis
): [number, number, number] {  const R = new THREE.Matrix4().makeRotationFromEuler(
