...
let sharedMaterials: { x: THREE.MeshBasicMaterial; y: THREE.MeshBasicMaterial; z: THREE.MeshBasicMaterial } | null = null
...
export class FrameVisualizer {
  ...
  /**
   * 增量同步：仅创建新增关节的坐标系、移除已删除的，
   * 已存在的复用同一批 Object3D（几何体与材质全局共享），
   * 避免每次 FK 更新都销毁重建全部箭头造成的 GC 抖动。
   */
