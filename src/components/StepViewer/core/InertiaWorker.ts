...
function meshMassProps(
  ...
  for (let t = 0; t < nTri; t++) {
    ...
    const ax = positions[i0] - rx, ay = positions[i0 + 1] - ry, az = positions[i0 + 2] - rz
    const bx = positions[i1] - rx, by = positions[i1 + 1] - ry, bz = positions[i1 + 2] - rz
    const cx = positions[i2] - rx, cy = positions[i2 + 1] - ry, cz = positions[i2 + 2] - rz
    ...
    neumaierAdd(sums, comps, 7, w * (2 * ax * ay + 2 * bx * by + 2 * cx * cy
      + ax * by + ay * bx + ax * cy + ay * cx + bx * cy + by * cx))
    neumaierAdd(sums, comps, 8, w * (2 * ax * az + 2 * bx * bz + 2 * cx * cz
      + ax * bz + az * bx + ax * cz + az * cx + bx * cz + bz * cx))
    neumaierAdd(sums, comps, 9, w * (2 * ay * az + 2 * by * bz + 2 * cy * cz
