...
import type {
  ...
  MeshImportOptions,
  MeshImportResult,
} from "./MeshImportWorker";
import type { SerializedSolidData, UploadProgress } from "../types";

let worker: Worker | null = null;
let workerProxy: Comlink.Remote<MeshImportWorkerApi> | null = null;
let initPromise: Promise<void> | null = null;

async function getProxy(): Promise<Comlink.Remote<MeshImportWorkerApi>> {
  if (!workerProxy) {
    worker = new Worker(new URL("./MeshImportWorker.ts", import.meta.url), { type: "module" });
    workerProxy = Comlink.wrap<MeshImportWorkerApi>(worker);
  }
  if (!initPromise) {
...
export async function importStlSolids(
  ...
  const callback = onProgress
    ? Comlink.proxy((stage: string, percent: number) => {
        onProgress({ status: "parsing", progress: Math.min(Math.round(percent), 99), message: stage });
      })
    : undefined;
